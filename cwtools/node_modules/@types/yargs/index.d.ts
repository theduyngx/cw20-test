...
declare namespace yargs {
    ...
    interface Argv<T = {}> {
         ...
         * Each key of this object should be the canonical version of the option, and each value should be a string or an
         * array of strings.
         */
        ...
        /**
         * Interpret `key` as a boolean. If a non-flag option follows `key` in `process.argv`, that string won't get set as 
         * the value of `key`.
         *
         ...
         */
        choices<K extends keyof T, C extends ReadonlyArray<any>>(key: K, values: C):
            Argv<Omit<T, K> & { [key in K]: C[number] | undefined }>;
        choices<K extends string, C extends ReadonlyArray<any>>(key: K, values: C):
            Argv<T & { [key in K]: C[number] | undefined }>;
        choices<C extends { [key: string]: ReadonlyArray<any> }>(choices: C):
         ...
         *
         * The coercion function should accept one argument, representing the parsed value from the command line, and should
         * return a new value or throw an error. The returned value will be used as the value for `key` (or one of its aliases)
         * in `argv`.
         *
         ...
         *
         * You can also map the same function to several keys at one time. Just pass an array of keys as the first argument
         * to `.coerce()`.
         *
         * If you are using dot-notion or arrays, .e.g., `user.email` and `user.password`, coercion will be applied to the final
         * object that has been parsed
         */
        coerce<K extends keyof T, V>(key: K | ReadonlyArray<K>, func: (arg: any) => V):
            Argv<Omit<T, K> & { [key in K]: V | undefined }>;
        coerce<K extends string, V>(key: K | ReadonlyArray<K>, func: (arg: any) => V):
            Argv<T & { [key in K]: V | undefined }>;
        coerce<O extends { [key: string]: (arg: any) => any }>(opts: O):
            Argv<Omit<T, keyof O> & { [key in keyof O]: ReturnType<O[key]> | undefined }>;

        /**
         * Define the commands exposed by your application.
         * @param command     Should be a string representing the command or an array of strings representing the command &
         *                    its aliases.
         * @param description Provide a description for each command your application accepts (the values stored in `argv._`
         *                    Set `description` to false to create a hidden command. Hidden commands don't show up in help
         *                    output and aren't available for completion.
         * @param [builder]   Object to give hints about the options that your command accepts. Can also be a function.
         *                    This function is executed with a yargs instance, and can be used to provide advanced command
         *                    specific help.
         ...
         *        To enable bash/zsh completions, concat the generated script to your `.bashrc` or `.bash_profile`
         *        (or `.zshrc` for zsh).
         * @param [description] Provide a description in your usage instructions for the command that generates the completion
         *        scripts.
         * @param [func] Rather than relying on yargs' default completion functionality, which shiver me timbers is pretty
         *        awesome, you can provide your own completion method.
         */
        completion(): Argv<T>;
        ...
        /**
         * Tells the parser that if the option specified by `key` is passed in, it should be interpreted as a path to a
         * JSON config file.
        ...
        /**
         * Given the key `x` is set, the key `y` must not be set. `y` can either be a single string or an array of argument
         * names that `x` conflicts with.
         *
        ...
        /**
         * Interpret `key` as a boolean flag, but set its parsed value to the number of flag occurrences rather than `true`
         * or `false`. Default value is thus `0`.
         */
         ...
         * The default value can be a `function` returning a value. The name of the function will be used in usage string.
         *
         ...
         * @param msg    If string is given, it will be printed when the argument is missing, instead of the standard err msg.
         * @param demand Controls whether the option is demanded; this is useful when using .options() to specify command
         *               line parameters.
         */
         ...
         * You can demand a minimum and a maximum number a user can have within your program, as well as provide corresponding
         * error messages if either of the demands is not met.
         */
        ...
        /**
         * Tell yargs to parse environment variables matching given prefix and apply them to argv as though they were
         * command line arguments.
         *
         * Use "__" separator in the env variable to indicate nested options. (e.g. prefix_nested__foo => nested.foo)
         * If this method is called with no argument / an empty string / true, then all env vars will be applied to argv.
         *
         ...
         * Inside `cmd`, the string `$0` will get interpolated to the current script name or node command for the present
         * script similar to how `$0` works in bash or perl.
        ...
        /** 
         * Manually indicate that program should exit, and provide context about why we wanted to exit. Follows behaviors
         * set by `.exitProcess().`
         */
         ...
         * validation fails, or the command handler fails.
         ...
         * @param func Is called with the failure message that would have been printed, the Error instance originally thrown
         *             and yargs state when the failure occurred.
         */
        ...
        /** Hides a key from the generated usage information. Unless a `--show-hidden` option is also passed with `--help`
         * (see `showHidden()`).
         */
         ...
         *
         * Note: any multi-char aliases (e.g. `help`) used for the help option will also be used for the implicit command.
         * If there are no multi-char aliases (e.g. `h`), then all single-char aliases will be used for the command.
         *
         * If invoked without parameters, `.help()` will use `--help` as the option and help as implicit command to trigger
         * help output.
         *
         ...
         * y` can either be the name of an argument to imply, a number indicating the position of an argument or an array of
         * multiple implications to associate with `x`.
         *
         ...
         *
         * By default, yargs will auto-detect the operating system's locale so that yargs-generated help content will display
         ...
         * @param callbacks               Can be a function or list of functions. Each callback gets passed a reference to argv.
         * @param [applyBeforeValidation] Set to `true` to apply middleware before validation. This will execute the
         *                                middleware prior to validation checks, but after parsing.
         */
        middleware(
         ...
         * You can also pass an opt object which can hold further customization, like `.alias()`, `.demandOption()` etc.
         * for that option.
         */
        option<K extends keyof T, O extends Options>(key: K, options: O):
            Argv<Omit<T, K> & { [key in K]: InferredOptionType<O> } & Alias<O>>;
        option<K extends string, O extends Options>(key: K, options: O):
            Argv<T & { [key in K]: InferredOptionType<O> } & Alias<O>>;
        option<O extends { [key: string]: Options }>(options: O):
         ...
         * for that option.
         */
        options<K extends keyof T, O extends Options>(key: K, options: O):
            Argv<Omit<T, K> & { [key in K]: InferredOptionType<O> }>;
        options<K extends string, O extends Options>(key: K, options: O):
        ...
        /**
         * Parse `args` instead of `process.argv`. Returns the `argv` object. `args` may either be a pre-processed argv array, 
         * or a raw argument string.
         *
        ...
        parseSync(arg: string | ReadonlyArray<string>, context?: object, parseCallback?: ParseCallback<T>): {
            [key in keyof Arguments<T> as key | CamelCaseKey<key>]: Arguments<T>[key]
        };
        parseAsync(): Promise<{ [key in keyof Arguments<T> as key | CamelCaseKey<key>]: Arguments<T>[key] }>;
        parseAsync(arg: string | ReadonlyArray<string>, context?: object, parseCallback?: ParseCallback<T>): Promise<{
            [key in keyof Arguments<T> as key | CamelCaseKey<key>]: Arguments<T>[key]
        }>;
        ...
        /**
         * Similar to `config()`, indicates that yargs should interpret the object from the specified key in package.json
         ...
         * `.positional()` should be called in a command's builder function, and is not available on the top-level yargs
         * instance. If so, it will throw an error.
         */
        positional<K extends keyof T, O extends PositionalOptions>(key: K, opt: O): Argv<Omit<T, K> & {
            [key in K]: InferredOptionType<O>
        }>;
         ...
         * Users of your application can install this script in their `.bashrc`, and yargs will provide completion
         * shortcuts for commands and options.
         */
        showCompletionScript(): Argv<T>;

        /**
         * Configure the `--show-hidden` option that displays the hidden keys (see `hide()`).
         * @param option      If `boolean`, it enables/disables this altogether. i.e. hidden keys will be permanently
         *                    hidden if first argument is `false`.
        ...
        /** Specifies either a single option key (string), or an array of options. If any of the options is present,
         * yargs validation is skipped. */
         ...
         * leading zeros in an input.
         *
         * If `key` is an array, interpret all the elements as strings.
         *
         * `.string('_')` will result in non-hyphenated arguments being interpreted as strings, regardless of whether they
         * resemble numbers.
         */
         ...
         * Inside `message`, the string `$0` will get interpolated to the current script name or node command for the present
         * script similar to how `$0` works in bash or perl.
         *
         * If the optional `description`/`builder`/`handler` are provided, `.usage()` acts an an alias for `.command()`.
         * This allows you to use `.usage()` to configure the default command that will be run as an entry-point to your
         * application and allows you to provide configuration for the positional arguments accepted by your program:
         */
        usage(message: string): Argv<T>;
        usage<U>(
            command: string | ReadonlyArray<string>, 
            description: string, builder?: (args: Argv<T>) 
                => Argv<U>, handler?: (args: ArgumentsCamelCase<U>)
                => void | Promise<void>
        ): Argv<T>;
        usage<U>(
            command: string | ReadonlyArray<string>, 
            showInHelp: boolean, builder?: (args: Argv<T>) 
                => Argv<U>, handler?: (args: ArgumentsCamelCase<U>) 
                => void | Promise<void>
        ): Argv<T>;
        ...
        /**
         * Add an option (e.g. `--version`) that displays the version number (given by the version parameter) and exits
         * the process. By default yargs enables version for the `--version` option.
         *
         * If no arguments are passed to version (`.version()`), yargs will parse the package.json of your module and use
         * its version value.
         *
         ...
         *
         * By default wrap will be set to `Math.min(80, windowWidth)`. Use `.wrap(null)` to specify no column limit (no
         * right-align). Use `.wrap(yargs.terminalWidth())` to maximize the width of yargs' usage instructions.
         */
    ...
    interface CommandModule<T = {}, U = {}> {
        /** 
         * array of strings (or a single string) representing aliases of `exports.command`, positional args defined
         * in an alias are ignored
         */
        aliases?: ReadonlyArray<string> | string | undefined;
        /** object declaring the options the command accepts, or a function accepting and returning a yargs instance */
        builder?: CommandBuilder<T, U> | undefined;
        /** 
         * string (or array of strings) that executes this command when given on the command line, first string may
         * contain positional args 
         */
    ...
    type CommandBuilder<T = {}, U = {}> = {
        [key: string]: Options } | ((args: Argv<T>) => Argv<U>) | ((args: Argv<T>) => PromiseLike<Argv<U>>);
    ...
    type FallbackCompletionFunction = (
        current: string, 
        argv: any, 
        completionFilter: (onCompleted?: CompletionCallback) => any, done: (completions: string[]) => any
    ) => void;
